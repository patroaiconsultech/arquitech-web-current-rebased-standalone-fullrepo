...
import { clearSession, fetchCurrentTermsVersion, getTenant, getToken, getUser, isAdmin, isApproved, setSession, logout } from "../lib/auth.js";
...
function formatActiveAgentRuntime(agentName = "") {
  ...
  if (!slug) return "";
  if (slug.startsWith("aria")) return "Aria respondendo";
...
function buildArquitechFallbackAuthUrl() {
  return "/auth?source=arquitech&agent=aria&product=arquitech&onboarding=1";
}
...
export default function AppConsole() {
  ...
  const isArquitechMode = readArquitechConsoleMode();
...
const messagesEndRef = useRef(null);
  ...
  const [legalVersionError, setLegalVersionError] = useState("");
    ...
    try {
      ...
      if (data) {
        ...
        try {
          ...
          const acceptedTermsVersion = String(mergedUser?.terms_version || "").trim();
          if (
            !mergedUser?.terms_accepted_at
            || acceptedTermsVersion !== String(currentTermsVersion || "").trim()
          ) {
          ...
          setLegalVersionError(
            legalError?.message
              || "Não foi possível validar a versão jurídica vigente. Tente novamente.",
          );
          setShowTermsModal(true);
        }
      }
    ...
    try {
      ...
      if (isArquitechMode) {
        ...
        if (aria?.id) {
          ...
          setDestMulti([]);
          try {
            window.localStorage?.setItem("orkio_last_dest_mode", "single");
            window.localStorage?.setItem("orkio_last_dest_single", aria.id);
            window.localStorage?.setItem("orkio_last_dest_multi", "[]");
          } catch {}
          return;
        }
      }
...
async function sendMessage(presetMsg = null, opts = {}) {
    ...
    try {
      ...
      const finalMsg = isArquitechMode
        ? (String(msg || "").replace(/@([A-Za-z0-9_\-/]+(?:\s+[A-Za-z0-9_\-/]+){0,2})(?=(?:\s*[,.:;!?])|(?:\s+@)|$)/gi, "").replace(/\s{2,}/g, " ").trim() || String(msg || "").trim())
        : pref + msg;
      ...
      setActiveRuntimeAgent(glipAssistantName);
        ...
        try {
          ...
          streamMeta = await withTimeout(consumeChatStream(streamResp, {
            ...
            onStatus: (payload) => {
              ...
              if (payload?.agent_name || payload?.final_speaker || payload?.visible_agent) setActiveRuntimeAgent(coerceConsoleAgentName(resolveAssistantDisplayName(payload, activeRuntimeAgent || "Agent")));
            ...
            onAgentDone: (payload) => {
              ...
              appendExecutionTrace({
                kind: "agent",
                label: `${coerceConsoleAgentName(resolveAssistantDisplayName(payload, payload?.agent_id || "Agent"))} concluiu uma etapa`,
            ...
            onDone: (payload) => {
              ...
              if (payload?.agent_name || payload?.final_speaker || payload?.visible_agent) {
                setActiveRuntimeAgent(coerceConsoleAgentName(resolveAssistantDisplayName(payload, activeRuntimeAgent || "Agent")));
              }
          ...
          if (isAbortLikeError(streamErr)) {
            ...
            if (hasPersistedAssistantForTurn(reconciledMessages, turnStartedAt)) {
              ...
              appendExecutionTrace({
                ...
                detail: "O histórico ainda não tinha resposta persistida. A resposta direta será tentada com timeout controlado.",
              });
              try {
            ...
            appendExecutionTrace({
              ...
              detail: "O stream retornou 429. Nenhum fallback duplicado foi acionado; tente novamente em alguns instantes.",
            });
            setMessages((prev) =>
              (Array.isArray(prev) ? prev : []).map((m) =>
                m.id === draftAssistantId
            ...
            appendExecutionTrace({
              ...
              detail: streamErr?.status === 403
                ? "Nenhum fallback duplicado foi acionado após 403."
                : "Nenhum fallback duplicado foi acionado após 401. Validando a sessão atual.",
            });
            setMessages((prev) =>
              (Array.isArray(prev) ? prev : []).map((m) =>
                m.id === draftAssistantId
            ...
            appendExecutionTrace({
              kind: "system",
              label: "Alternando para resposta direta",
              detail: "O stream foi degradado. A resposta direta será tentada com timeout controlado.",
            });
            try {
              resp = await runDirectChat();
            } catch (fallbackErr) {
      ...
      if (!freshHasAssistant && finalTextCandidate) {
        setMessages((prev) => {
          ...
          const preservedDraft = {
            ...
            agent_name: finalAgentName || glipAssistantName,
            agent_id: finalAgentId || null,
            voice_id: finalVoiceId || null,
            avatar_url: finalAvatarUrl || null,
...
async function stopRealtime(reason = 'client_stop') {
  ...
  return (
    ...
    {showTermsModal && (
      <TermsModal
        productLabel={isArquitechMode ? "GLIP Intelligence Architecture" : "Orkio"}
        initialError={legalVersionError}
        onAccepted={async (result = {}) => {
        setShowTermsModal(false);
        setLegalVersionError("");
        const resolvedTermsVersion =
          result?.termsVersion || await fetchCurrentTermsVersion();
    ...
    <div style={styles.layout}>
      ...
      <div style={{ ...styles.sidebar, display: (!isMobile || mobileSidebarOpen) ? "flex" : "none", position: isMobile ? "fixed" : styles.sidebar.position, inset: isMobile ? "0 auto 0 0" : "auto", width: isMobile ? "min(88vw, 360px)" : styles.sidebar.width, zIndex: isMobile ? 40 : styles.sidebar.zIndex, boxShadow: isMobile ? "0 24px 80px rgba(0,0,0,0.45)" : styles.sidebar.boxShadow, borderRight: isMobile ? "1px solid rgba(255,255,255,0.08)" : styles.sidebar.borderRight }}>
        <div style={styles.topRow}>
          <div>
            <div style={styles.brand}>{isArquitechMode ? "GLIP" : "Orkio"}</div>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: isMobile ? "wrap" : "nowrap", justifyContent: "flex-end" }}>
            ...
            {isArquitechMode ? (
              <div
                ...
                title="Modo GLIP: Aria é a coordenadora inteligente deste fluxo."
              >
                <span>GLIP</span>
              ...
              >
                ...
                {m.role === "assistant" && lastAgentInfo?.avatar_url && (
                  <div style={{ marginRight: 8, flexShrink: 0, alignSelf: "flex-start", marginTop: 4 }}>
                    <img
                      ...
                      alt={coerceConsoleAgentName(resolveAssistantDisplayName(m, "Agent"))}
                ...
                >
                  {(() => {
                    ...
                    const name = isUser
                      ? (m.user_name || meName)
                      : (isSystem ? "Sistema" : coerceConsoleAgentName(resolveAssistantDisplayName(m, "Agent")));
                    ...
                    const normalizedVisible = humanizeConsoleStatusMessage(normalizeUserFacingRuntimeMessage(visibleRaw || m.content));
